...
# ---------------------------------------------------------
# 2. 구글 API 연결 및 데이터 수집 (₩, $ 에러 수정 완벽 반영)
# ---------------------------------------------------------
@st.cache_data
def load_data():
...
# ---------------------------------------------------------
# 4. ⭐️ [변경점 1] 종목별 상세 현황 표 (+ 자산 비중 추가)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 5. ⭐️ [변경점 2] 목표가격 기반 시뮬레이터 (직전의 깔끔한 버전에 기능만 추가)
# ---------------------------------------------------------
st.subheader("🔮 2027년 내 집 마련 시뮬레이터 (목표가격 기반)")
st.write("💡 **'월 적립금'과 '목표가격' 칸을 더블클릭해서 수정해보세요!** 파이썬이 목표가격에 도달하기 위한 수익률을 역산하여 2027년 자산을 계산합니다.")

# 현재 보유 자산을 종목명 기준으로 합산하고 현재가 가져오기
...
for asset, val in current_assets.items():
    ...
    # 주식이나 코인처럼 현재가가 있는 종목은 현재가를 초기 목표가로 세팅
...
edited_df = st.data_editor(
    sim_input_df, 
    disabled=['자산/종목명', '현재 자산(원)', '현재가격'], # 이 칸들은 수정 불가
